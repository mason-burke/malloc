...
static inline void pull_free_block(block_t *fb) {
    ...
    // get the next and previous block in the flist
    block_t *next = block_flink(fb);
    block_t *prev = block_blink(fb);

    // if the block is the only block in the list, empty the list
    if (fb == flist_first && next == flist_first) {
        flist_first = NULL;
    } else {
        // update next and previous to point to each other
        block_set_blink(next, prev);
        block_set_flink(prev, next);

        // check if it's at the front of the list
        if (fb == flist_first) {
            flist_first = next;
        }
    }
